...
def handle_files(data, meta_data, info_data):
    ...
    for x in df1.sheet_names:
        ...
        dest_file_name = f'{x}{target_file_format}'
        ...
        if info_data_available:
            info_data.set_info(f'{x}{target_file_format} {status}')
    if archive_output:
        output_files_list_single_file = [
...
def process_data(data, meta_data, info_data, flip_output=False):
    ...
    res = handle_files(data=data, meta_data=meta_data, info_data=info_data)
    if flip_output is True:
